...
def create_kafka_producer() -> KafkaProducer:
    ...
    while retries > 0:
        try:
            ...
            log.warning(f"No Kafka brokers available. Retrying in 5s... ({retries} left)")
            retries -= 1
            time.sleep(5)
...
async def fetch_and_send(producer: KafkaProducer, symbol: str):
    ...
    try:
        ...
        if price is None or timestamp_s is None:
            log.warning(f"No 'currentPrice' or 'regularMarketTime' for {symbol}. Skipping. (Market might be closed)")
            return
...
async def main_loop():
    ...
    while True:
        ...
        tasks = [
            fetch_and_send(producer, symbol) 
