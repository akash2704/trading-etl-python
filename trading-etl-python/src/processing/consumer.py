    ...
    if records_to_insert:
        try:
            with db_conn.cursor() as cur:
                ...
                cols_str = cols_str.replace('"Open"', '"open"').replace(
                    '"Close"', '"close"'
                )
                ...
                execute_values(cur, sql_stmt, records_to_insert)
            db_conn.commit()
...
def main_loop():
    ...
    while True:
        try:
            ...
            log.info(
                f"Processed batch in {duration:.2f}s. Inserted {inserted_count} new indicator rows."
            )
