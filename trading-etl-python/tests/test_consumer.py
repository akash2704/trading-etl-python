...
def mock_ta():
    ...
    with (
        patch("src.processing.consumer.ta.sma", side_effect=mock_sma_func) as _,
...
async def test_consumer_warmup(mock_db_conn):
    ...
    messages = [
        create_fake_message("AAPL", 150 + i, 1700000000 + i * 5000) for i in range(10)
    ]
...
async def test_consumer_calculates_and_inserts(mock_db_conn, mock_ta):
    ...
    messages = [
        create_fake_message("AAPL", 150 + i, 1700000000 + i * 5000) for i in range(30)
    ]
